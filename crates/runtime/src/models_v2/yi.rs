...
impl YiConfig {
    /// Create YiConfig from GGUF model configuration
...
impl Model for YiModelV2 {
    ...
    fn forward(&self, inputs: &ModelInputs) -> Result<ModelOutputs> {
        match inputs {
            ...
            _ => Err(anyhow::anyhow!("Yi model only supports text and multimodal inputs")),
        }
    }
...
impl YiLayer {
    fn new(config: &YiConfig, device: &Device) -> Result<Self> {
        let self_attn = YiAttention::new(config, device)?;
        let mlp = YiMLP::new(config, device)?;
...
impl YiAttention {
    ...
    fn forward(&self, hidden_states: &Tensor, _attention_mask: Option<&Tensor>, rope_theta: f32) -> Result<Tensor> {
        ...
        // Yi uses aggressive GQA with 32:4 ratio (8 groups)
...
impl YiMLP {
    ...
    fn forward(&self, hidden_states: &Tensor) -> Result<Tensor> {
        ...
        // 2. Apply activation (SiLU for Yi)
...
mod tests {
    ...
    fn test_yi_model_creation() {
        ...
        let model = YiModelV2::new(config).unwrap();
    ...
    #[test]
    fn test_yi_gqa_ratio() {
        // Test that Yi's aggressive GQA (32:4 ratio) works correctly
        let config = YiConfig {
            vocab_size: 100,
            hidden_size: 128,
            intermediate_size: 256,
        ...
        assert_eq!(model.config().num_key_value_heads, 4);
        // GQA ratio should be 8
        assert_eq!(model.config().num_attention_heads / model.config().num_key_value_heads, 8);
    }
}
