...
model_config!(DeepSeekConfig {
    ...
    use_cache: bool = true,
    pad_token_id: i64 = 0,
    bos_token_id: i64 = 100000,
    eos_token_id: i64 = 100001,
...
impl DeepSeekConfig {
    /// Create DeepSeekConfig from GGUF model configuration
...
impl Model for DeepSeekModelV2 {
    ...
    fn forward(&self, inputs: &ModelInputs) -> Result<ModelOutputs> {
        match inputs {
            ...
            _ => Err(anyhow::anyhow!("DeepSeek model only supports text and multimodal inputs")),
        }
    }
...
impl DeepSeekLayer {
    fn new(config: &DeepSeekConfig, device: &Device) -> Result<Self> {
        let self_attn = DeepSeekAttention::new(config, device)?;
        let mlp = DeepSeekMLP::new(config, device)?;
...
impl DeepSeekMLP {
    ...
    fn forward(&self, hidden_states: &Tensor) -> Result<Tensor> {
        ...
        // 2. Apply activation (SiLU for DeepSeek)
...
mod tests {
    ...
    fn test_deepseek_model_creation() {
        ...
        let model = DeepSeekModelV2::new(config).unwrap();
