...
pub struct OlmoModelV2 {
    config: OlmoConfig,
    device: Device,
    embed_tokens: Tensor,
    layers: Vec<OlmoLayer>,
    norm: Tensor,
    lm_head: Tensor,
}

pub struct OlmoLayer {
...
impl Model for OlmoModelV2 {
    type Config = OlmoConfig;

    fn new(config: OlmoConfig) -> Result<Self> {
        ...
        for _ in 0..config.num_hidden_layers {
            layers.push(OlmoLayer::new(&config, &device)?);
        }

        Ok(Self { config, device, embed_tokens, layers, norm, lm_head })
    }

    fn from_weights(config: OlmoConfig, weights: ModelWeights) -> Result<Self> {
    ...
    fn forward(&self, inputs: &ModelInputs) -> Result<ModelOutputs> {
        match inputs {
            ...
            _ => Err(anyhow::anyhow!("OLMo only supports text inputs")),
        }
    }
...
impl OlmoLayer {
    fn new(config: &OlmoConfig, device: &Device) -> Result<Self> {
        Ok(Self {
            self_attn: OlmoAttention::new(config, device)?,
            mlp: OlmoMLP::new(config, device)?,
    ...
    fn forward(&self, hidden_states: &Tensor, seq_len: usize, rope_theta: f32) -> Result<Tensor> {
        ...
        let h = ops_fn::layer_norm(hidden_states, &self.input_layernorm, None, 1e-5)?;
        let attn_out = self.self_attn.forward(&h, seq_len, rope_theta)?;
        let h = ops_fn::add(&residual, &attn_out)?;

        let residual = h.clone();
        let h = ops_fn::layer_norm(&h, &self.post_attention_layernorm, None, 1e-5)?;
...
impl OlmoAttention {
    ...
    fn forward(&self, hidden_states: &Tensor, seq_len: usize, rope_theta: f32) -> Result<Tensor> {
        ...
        let (q, k) = apply_rope_olmo(&q, &k, seq_len, self.head_dim, rope_theta)?;
...
mod tests {
    use super::*;
    #[test]
    fn test_olmo_creation() {
        let config = OlmoConfig { vocab_size: 1000, hidden_size: 128, intermediate_size: 512, num_hidden_layers: 2, num_attention_heads: 4, num_key_value_heads: 4, ..Default::default() };
        let model = OlmoModelV2::new(config).unwrap();
        assert_eq!(model.config().vocab_size(), 1000);
    }
    #[test]
    fn test_olmo_forward() {
        let config = OlmoConfig { vocab_size: 100, hidden_size: 64, intermediate_size: 256, num_hidden_layers: 1, num_attention_heads: 4, num_key_value_heads: 4, ..Default::default() };
        let model = OlmoModelV2::new(config).unwrap();
