...
//!
//! DeepSeek-MoE architecture features:
//! - Fine-grained MoE with many experts (64-160)
//! - Shared experts that are always active
...
pub struct DeepSeekMoEBlock {
    router: Tensor,
    shared_experts: Vec<DeepSeekExpert>,
    routed_experts: Vec<DeepSeekExpert>,
...
impl Model for DeepSeekMoEModelV2 {
    type Config = DeepSeekMoEConfig;

    fn new(config: DeepSeekMoEConfig) -> Result<Self> {
        ...
        for i in 0..config.num_hidden_layers {
            layers.push(DeepSeekMoELayer::new(&config, &device, i)?);
        }

        Ok(Self { config, device, embed_tokens, layers, norm, lm_head })
    }

    fn from_weights(config: DeepSeekMoEConfig, weights: ModelWeights) -> Result<Self> {
    ...
    fn forward(&self, inputs: &ModelInputs) -> Result<ModelOutputs> {
        match inputs {
            ...
            _ => Err(anyhow::anyhow!("DeepSeek-MoE only supports text inputs")),
        }
    }
...
impl DeepSeekMoELayer {
    fn new(config: &DeepSeekMoEConfig, device: &Device, layer_idx: usize) -> Result<Self> {
        Ok(Self {
            self_attn: DeepSeekAttention::new(config, device)?,
            mlp: DeepSeekMoEBlock::new(config, device, layer_idx)?,
    ...
    fn forward(&self, hidden_states: &Tensor, seq_len: usize, rope_theta: f32) -> Result<Tensor> {
        ...
        let h = ops_fn::rms_norm(hidden_states, &self.input_layernorm, 1e-6)?;
        let attn_out = self.self_attn.forward(&h, seq_len, rope_theta)?;
        let h = ops_fn::add(&residual, &attn_out)?;

        let residual = h.clone();
        let h = ops_fn::rms_norm(&h, &self.post_attention_layernorm, 1e-6)?;
...
impl DeepSeekAttention {
    fn new(config: &DeepSeekMoEConfig, device: &Device) -> Result<Self> {
    ...
    fn forward(&self, hidden_states: &Tensor, seq_len: usize, rope_theta: f32) -> Result<Tensor> {
        ...
        let (q, k) = apply_rope(&q, &k, seq_len, self.head_dim, rope_theta)?;
...
impl DeepSeekMoEBlock {
    fn new(config: &DeepSeekMoEConfig, device: &Device, layer_idx: usize) -> Result<Self> {
        // First few layers use dense MLP, rest use MoE
        let use_moe = layer_idx >= config.first_k_dense_replace;

        let mut shared_experts = Vec::with_capacity(config.num_shared_experts);
        for _ in 0..config.num_shared_experts {
            shared_experts.push(DeepSeekExpert::new(config.hidden_size, config.moe_intermediate_size, device)?);
        }

        let mut routed_experts = Vec::new();
        if use_moe {
            for _ in 0..config.num_experts {
                routed_experts.push(DeepSeekExpert::new(config.hidden_size, config.moe_intermediate_size, device)?);
            }
        }

        let router = if use_moe {
            ops_fn::zeros(&[config.hidden_size, config.num_experts], DataType::Float32, device)?
        } else {
            ops_fn::zeros(&[1, 1], DataType::Float32, device)?
        };
    ...
    fn forward(&self, hidden_states: &Tensor) -> Result<Tensor> {
        ...
        let mut output = ops_fn::zeros(&[batch_size, seq_len, hidden_size], hidden_states.dtype(), hidden_states.device())?;
        for expert in &self.shared_experts {
            let expert_out = expert.forward(hidden_states)?;
            output = ops_fn::add(&output, &expert_out)?;
        }
        ...
        if !self.routed_experts.is_empty() {
            ...
            let mut routed_output_data = vec![0.0f32; num_tokens * hidden_size];
            ...
            for tok_idx in 0..num_tokens {
                ...
                for (i, &expert_idx) in indices.iter().enumerate() {
                    if (expert_idx as usize) < self.routed_experts.len() {
                        let expert = &self.routed_experts[expert_idx as usize];
                ...
                for (i, &v) in token_data.iter().enumerate() {
                    routed_output_data[tok_idx * hidden_size + i] = v;
                }
            }

            let routed_output = Tensor::from_f32_slice(&routed_output_data, &[num_tokens, hidden_size], hidden_states.device())?;
            let routed_output = routed_output.reshape(&[batch_size, seq_len, hidden_size])?;
            output = ops_fn::add(&output, &routed_output)?;
        }
    ...
    fn to_device(&mut self, device: &Device) -> Result<()> {
        ...
        for expert in &mut self.shared_experts { expert.to_device(device)?; }
        for expert in &mut self.routed_experts { expert.to_device(device)?; }
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_deepseek_moe_creation() {
        let config = DeepSeekMoEConfig { vocab_size: 1000, hidden_size: 64, intermediate_size: 256, moe_intermediate_size: 64, num_hidden_layers: 2, num_attention_heads: 4, num_key_value_heads: 4, num_experts: 4, num_experts_per_tok: 2, num_shared_experts: 1, first_k_dense_replace: 1, ..Default::default() };
        let model = DeepSeekMoEModelV2::new(config).unwrap();
        assert_eq!(model.config().vocab_size(), 1000);
    }
    #[test]
    fn test_deepseek_moe_forward() {
        let config = DeepSeekMoEConfig { vocab_size: 100, hidden_size: 64, intermediate_size: 256, moe_intermediate_size: 64, num_hidden_layers: 2, num_attention_heads: 4, num_key_value_heads: 4, num_experts: 4, num_experts_per_tok: 2, num_shared_experts: 1, first_k_dense_replace: 1, ..Default::default() };
        let model = DeepSeekMoEModelV2::new(config).unwrap();
