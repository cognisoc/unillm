...
model_config!(IdeficsConfig {
    ...
    cross_attention_frequency: usize = 4,
    pad_token_id: i64 = 0,
    bos_token_id: i64 = 1,
    eos_token_id: i64 = 2,
});
...
impl IdeficsConfig {
    ...
    pub fn is_cross_attention_layer(&self, layer_idx: usize) -> bool {
        layer_idx % self.cross_attention_frequency == 0
    }
}

pub struct IdeficsModelV2 {
    config: IdeficsConfig,
    device: Device,
    embed_tokens: Tensor,
    layers: Vec<IdeficsLayer>,
    norm: Tensor,
    lm_head: Tensor,
}

pub struct IdeficsLayer {
    self_attn_q: Tensor,
    self_attn_k: Tensor,
    self_attn_v: Tensor,
    self_attn_o: Tensor,
...
impl Model for IdeficsModelV2 {
    type Config = IdeficsConfig;

    fn new(config: IdeficsConfig) -> Result<Self> {
        ...
        for _ in 0..config.num_hidden_layers {
            layers.push(IdeficsLayer::new(&config, &device)?);
        }

        Ok(Self { config, device, embed_tokens, layers, norm, lm_head })
    }

    fn from_weights(config: IdeficsConfig, weights: ModelWeights) -> Result<Self> {
    ...
    fn forward(&self, inputs: &ModelInputs) -> Result<ModelOutputs> {
        match inputs {
            ...
            _ => Err(anyhow::anyhow!("Idefics requires text input")),
        }
    }
...
impl IdeficsLayer {
    fn new(config: &IdeficsConfig, device: &Device) -> Result<Self> {
    ...
    fn forward(&self, hidden_states: &Tensor) -> Result<Tensor> {
        ...
        let k = ops_fn::matmul(&hidden, &self.self_attn_k)?.to_candle()?.reshape(&[batch_size, seq_len, self.num_kv_heads, self.head_dim])?.transpose(1, 2)?;
        let v = ops_fn::matmul(&hidden, &self.self_attn_v)?.to_candle()?.reshape(&[batch_size, seq_len, self.num_kv_heads, self.head_dim])?.transpose(1, 2)?;
...
mod tests {
    ...
    #[test]
    fn test_idefics_forward() {
        let config = IdeficsConfig { vocab_size: 100, hidden_size: 32, intermediate_size: 128, num_hidden_layers: 1, num_attention_heads: 2, num_key_value_heads: 2, ..Default::default() };
        let model = IdeficsModelV2::new(config).unwrap();
