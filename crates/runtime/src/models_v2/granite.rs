...
//!
//! Granite (IBM's decoder model) features:
...
impl Model for GraniteModelV2 {
    type Config = GraniteConfig;

    fn new(config: GraniteConfig) -> Result<Self> {
        ...
        for _ in 0..config.num_hidden_layers {
            layers.push(GraniteLayer::new(&config, &device)?);
        }

        Ok(Self { config, device, embed_tokens, layers, norm, lm_head })
    }

    fn from_weights(config: GraniteConfig, weights: ModelWeights) -> Result<Self> {
        ...
        match inputs {
            ModelInputs::Text { input_ids, .. } => {
                ...
                for layer in &self.layers {
                    hidden = layer.forward(&hidden, seq_len, self.config.rope_theta, self.config.residual_multiplier)?;
                }
            ...
            _ => Err(anyhow::anyhow!("Granite only supports text inputs")),
        }
    }
...
impl GraniteLayer {
    fn new(config: &GraniteConfig, device: &Device) -> Result<Self> {
        Ok(Self {
            self_attn: GraniteAttention::new(config, device)?,
            mlp: GraniteMLP::new(config, device)?,
    ...
    fn forward(&self, hidden_states: &Tensor, seq_len: usize, rope_theta: f32, residual_multiplier: f32) -> Result<Tensor> {
        ...
        let attn_scaled = if residual_multiplier != 1.0 {
            ops_fn::scale(&attn_out, residual_multiplier)?
        } else {
            attn_out
        };
        let h = ops_fn::add(&residual, &attn_scaled)?;

        let residual = h.clone();
        let h = ops_fn::rms_norm(&h, &self.post_attention_layernorm, 1e-5)?;
        let mlp_out = self.mlp.forward(&h)?;
        let mlp_scaled = if residual_multiplier != 1.0 {
            ops_fn::scale(&mlp_out, residual_multiplier)?
        } else {
            mlp_out
        };
...
impl GraniteAttention {
    ...
    fn forward(&self, hidden_states: &Tensor, seq_len: usize, rope_theta: f32) -> Result<Tensor> {
        ...
        let (q, k) = apply_rope_granite(&q, &k, seq_len, self.head_dim, rope_theta)?;
...
mod tests {
    use super::*;
    #[test]
    fn test_granite_creation() {
        let config = GraniteConfig { vocab_size: 1000, hidden_size: 128, intermediate_size: 512, num_hidden_layers: 2, num_attention_heads: 4, num_key_value_heads: 4, ..Default::default() };
        let model = GraniteModelV2::new(config).unwrap();
        assert_eq!(model.config().vocab_size(), 1000);
    }
    #[test]
    fn test_granite_forward() {
        let config = GraniteConfig { vocab_size: 100, hidden_size: 64, intermediate_size: 256, num_hidden_layers: 1, num_attention_heads: 4, num_key_value_heads: 4, ..Default::default() };
        let model = GraniteModelV2::new(config).unwrap();
