...
//!
//! MiniCPM unique features:
//! - scale_emb scaling on embeddings
//! - scale_depth / sqrt(num_layers) scaling per layer
//! - tied embeddings with transpose for lm_head
...
impl MiniCPMConfig {
    /// Create MiniCPMConfig from GGUF model configuration
...
impl Model for MiniCPMModelV2 {
    ...
    fn new(config: MiniCPMConfig) -> Result<Self> {
        ...
        let lm_head = if config.tie_word_embeddings {
            ...
            Some(ops_fn::zeros(&[config.hidden_size, config.vocab_size], DataType::Float32, &device)?)
        };
    ...
    fn from_weights(config: MiniCPMConfig, weights: ModelWeights) -> Result<Self> {
        ...
        // Load embedding weights (no transpose - used for index lookup)
        ...
        // For tied embeddings, lm_head is None and we use embed_tokens transposed at inference
        // For separate lm_head, transpose: [vocab, hidden] -> [hidden, vocab] for matmul
    ...
    fn forward(&self, inputs: &ModelInputs) -> Result<ModelOutputs> {
        ...
        for layer in &self.layers {
            hidden_states = layer.forward(&hidden_states, None, self.config.rope_theta)?;
        }
        ...
        let logits = if let Some(ref lm_head) = self.lm_head {
            // Separate lm_head (already transposed)
            ops_fn::matmul(&hidden_states, lm_head)?
        } else {
            // Tied embeddings - need to transpose embed_tokens for matmul
            // embed_tokens: [vocab, hidden] -> need [hidden, vocab]
            let lm_head_t = ops_fn::transpose(&self.embed_tokens)?;
            ops_fn::matmul(&hidden_states, &lm_head_t)?
        };

        // Apply final scaling (MiniCPM specific - inverse of embedding scale)
...
impl MiniCPMDecoderLayer {
    fn new(config: &MiniCPMConfig, device: &Device, _layer_idx: usize) -> Result<Self> {
    ...
    fn forward(&self, hidden_states: &Tensor, _attention_mask: Option<&Tensor>, rope_theta: f32) -> Result<Tensor> {
        ...
        let attn_output = self.self_attn.forward(&normed, None, rope_theta)?;
...
impl MiniCPMAttention {
    fn new(config: &MiniCPMConfig, device: &Device) -> Result<Self> {
        ...
        Ok(Self {
            ...
            k_proj: ops_fn::zeros(&[config.hidden_size, num_key_value_heads * head_dim], DataType::Float32, device)?,
            v_proj: ops_fn::zeros(&[config.hidden_size, num_key_value_heads * head_dim], DataType::Float32, device)?,
...
impl MiniCPMMLP {
    ...
    fn forward(&self, hidden_states: &Tensor) -> Result<Tensor> {
        ...
        // 2. Apply activation (SiLU for MiniCPM)
...
mod tests {
    ...
    #[test]
    fn test_minicpm_model_creation() {
        let config = MiniCPMConfig {
            vocab_size: 1000,
            hidden_size: 128,
        ...
        let model = MiniCPMModelV2::new(config).unwrap();
    ...
    #[test]
    fn test_minicpm_gqa() {
        // Test with GQA (num_key_value_heads < num_attention_heads)
        let config = MiniCPMConfig {
            vocab_size: 100,
            hidden_size: 64,
            intermediate_size: 256,
